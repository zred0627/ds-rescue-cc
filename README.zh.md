...
> ds-rescue 是一个单一二进制 CLI，使用 DeepSeek API 为 Claude Code（及其他 LLM 编码工具）提供独立的第二意见——review 方案、方案比选和提交，成本比 Codex 低 50 倍，彻底消除回音壁。
...
- **成本**：每次 review 约 $0.01（DeepSeek v4-pro），对比约 $0.30–0.60（Codex GPT-5 high）
...
- 你想要跨家族 LLM review，但不想为每次调用支付 $0.30–0.60

### 不适合的场景

- 你需要聊天助手或代码生成器——直接用 Claude / GPT / Gemini
...
实际限制在于成本。按典型方案或提交体量，单次 Codex review 调用大约花费 $0.30–0.60 USD。偶尔使用——每周的架构 review、上线前审计——这个成本完全可以接受。但贵到让大多数开发者不会在每份方案、每次提交、每次方案比选时都跑一遍。**习惯无法形成。**

开发者工具的习惯形成，要求多用一次的边际成本低到可以忽略不计。ds-rescue 建立在一条不同的成本曲线上。

### 成本对比

| 工具 | 单次 review 成本 | 每月（每天 10 次） | 习惯化？ |
|---|---|---|---|
| Codex GPT-5 high | $0.30–0.60 | $90–180 | 否——太贵，难以日常化 |
| ds-rescue（DeepSeek v4-pro） | ~$0.01 | ~$3 | 是——每份方案和提交都跑 |
| ds-rescue（DeepSeek v4-flash） | ~$0.002 | ~$0.60 | 是——每次 prompt 迭代都跑 |

在 $0.01 一次 review 的情况下，习惯性使用的经济门槛实际上消失了。

### 为什么选 DeepSeek

- **跨家族互补性**：在与 OpenAI 或 Anthropic 模型不同的数据分布上训练，盲点不重叠
- **中文领域推理深度**：对于使用中文工作、或处理制造/物流/政策问题的团队，DeepSeek 的中文推理质量明显强于大多数西方来源的模型——这在某些领域是首要差异化点
- **SWE-Bench 质量**：接近 GPT-4 级别的编程基准表现，足以抓住真实的 bug
...
| 模型家族 | OpenAI（GPT-5） | DeepSeek（v4-pro / v4-flash / R1） |
| 单次 review 成本 | $0.30–0.60 | ~$0.01（pro）/ ~$0.002（flash） |
...
不支持，这是设计决策。跨家族 review 的价值，正来自于使用与生成工件的模型**不同**的模型家族。如果你想对 Claude 撰写的方案做 DeepSeek review，ds-rescue 做到了。如果你想要 GPT-5 review，请用 codex-plugin-cc。
...
具体计算：每天 10 次 review × $0.01 = $0.10/天 = 约 $3/月。相当于每月一杯咖啡，或者相同频率下 Codex 成本（$90–180/月）的不到 2%。

使用 flash 模型：每天 10 次 × $0.002 = $0.60/月。
...
| `reasoner` | deepseek-reasoner（R1） | 深度链式推理 |
| `chat` | deepseek-chat（旧版） | 兼容性别名 |

使用 `--model pro`、`--model flash` 或 `--model reasoner` 选择。
...
| `--model {pro\|flash\|reasoner}` | `pro` | DeepSeek 模型别名 |
